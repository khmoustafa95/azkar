class AppConstants {
  /// Display name (launcher, app bar, share).
  static const String appName = 'ماسة القلوب';

  /// Longer branding line for about/contact copy.
  static const String appFullName = 'ماسة القلوب للحج والعمرة';
  static const String googlePlayUrl =
      'https://play.google.com/store/apps/details?id=net.quran.holly';
  static const String whatsPhone = '+905511859843';
  static const int expandSpeedTime = 10;
  static const int tasbihSpeedTime = 500;
  static const int snackBarSpeedTime = 800;
  static const int dayWheelSpeed = 1000;
  static const int souraAppBarView = 5;
}
